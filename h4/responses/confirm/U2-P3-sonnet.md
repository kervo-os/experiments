...
전제: 저장소 코드에는 접근하지 못했고, `/Users/threat-intel/Developer/30_lab/EXPER/h4-kit/artifact-P3.md`(kervo 자동 생성 컨텍스트 아티팩트) 한 건만 근거로 작성함. 이 아티팩트는 "Fact" 절(저장소 스캔 기반, 결정론적)과 "Slot" 절(신뢰 라벨이 붙은 관찰 — verified/generated/observed/stale)이 혼재되어 있으므로, 아래 각 진술마다 근거와 신뢰 라벨을 표기한다. 문서에 없는 내용은 추정하지 않고 "불명"으로 남긴다.
